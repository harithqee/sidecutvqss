<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
    ...
    <div class="flex items-end justify-between mt-5">
      <div>
        ...
        <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">
          3,782 <span class="ml-2 text-sm font-medium text-gray-500 dark:text-gray-400">In Queue</span>
        </h4>
      </div>
  ...
  <!-- Card 2: Total in Queue with 1:4 Ratio List -->
  <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
  ...
  <div class="flex items-center gap-4 mb-6">
    ...
    <div>
      <h4 class="text-title-sm font-bold text-gray-800 dark:text-white/90">Barber Status</h4>
      <span class="text-sm text-gray-500 dark:text-gray-400">Current availability</span>
    </div>
  </div>

  <!-- Full Width List (Grid and Left Border Removed) -->
  <div class="flex flex-col gap-5">
    
    <!-- Barber 1 (Defaults to Active) -->
    <div class="flex items-center justify-between gap-3" x-data="{ switcherToggle: true }">
      <!-- Left side: Avatar & Name -->
      <div class="flex items-center gap-3">
        <img src="./images/user/user-17.jpg" alt="Lindsey Curtis" class="h-11 w-11 shrink-0 rounded-full object-cover shadow-sm" />
        <div class="flex flex-col">
          <span class="whitespace-nowrap text-base font-bold text-gray-900 dark:text-white">Lindsey Curtis</span>
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Senior Barber</span>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <span class="flex shrink-0 items-center gap-2 text-sm font-medium transition-colors duration-300"
              :class="switcherToggle ? 'text-green-500' : 'text-red-500'">
        ...
        <!-- Alpine Toggle -->
        <label for="toggle-barber-1" class="flex cursor-pointer items-center select-none">
            <div class="relative">
                <!-- Added :checked to keep input synced -->
                <input type="checkbox" id="toggle-barber-1" class="sr-only" @change="switcherToggle = !switcherToggle" :checked="switcherToggle" />
    ...
    <!-- Barber 2 (Defaults to Inactive) -->
    <div class="flex items-center justify-between gap-3" x-data="{ switcherToggle: false }">
      <!-- Left side: Avatar & Name -->
      <div class="flex items-center gap-3">
        <img src="./images/user/user-18.jpg" alt="Danial" class="h-11 w-11 shrink-0 rounded-full object-cover shadow-sm" />
        <div class="flex flex-col">
          <span class="whitespace-nowrap text-base font-bold text-gray-900 dark:text-white">Danial</span>
          <span class="text-xs font-medium text-gray-500 dark:text-gray-400">Junior Barber</span>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4">
        ...
        <!-- Alpine Toggle -->
        <label for="toggle-barber-2" class="flex cursor-pointer items-center select-none">
            <div class="relative">
                <input type="checkbox" id="toggle-barber-2" class="sr-only" @change="switcherToggle = !switcherToggle" :checked="switcherToggle" />
                <div class="block h-6 w-11 rounded-full transition-colors duration-300"
                    :class="switcherToggle ? 'bg-green-500' : 'bg-red-500'">
                </div>
                <div :class="switcherToggle ? 'translate-x-full' : 'translate-x-0'"
